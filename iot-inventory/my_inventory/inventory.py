...
@dataclass
class FridgeInventoryItem(InventoryItem):
    device_name: str = "fridge"

    @property
    def attrs(cls):
        return {
            "friendly_name": f"Fridge - {cls.unique_id.replace('_price', '').replace('_', ' ').title()} {cls.device_name} Item Number",
            "icon": "mdi:fridge"
        }


@dataclass
class FrozenInventoryItem(InventoryItem):
    device_name: str = "freezer"

    @property
    def attrs(cls):
        return {
            "friendly_name": f"Frozen - {cls.unique_id.replace('_price', '').replace('_', ' ').title()} {cls.device_name} Item Number",
...
@dataclass  # if item includes multiple units, eg a 6 pack of beer
class ItemPricePerUnit(ItemPrice):

    @property
    def attrs(cls):
        return {
            "friendly_name": f"{cls.unique_id.replace('_price', '').replace('_', ' ').title()} {cls.device_name} Price per unit",
            "icon": "mdi:currency-eur",
            "unit_of_measurement": cls.unit
        }
...
@dataclass  # eg, eur per liter, eur per kg
class ItemPricePerCapacity(ItemPrice):

    @property
    def attrs(cls):
        return {
            "friendly_name": f"{cls.unique_id.replace(f'_price_per_{_norm(cls.unit)}', '').replace('_', ' ').title()} {cls.device_name} Price per {cls.unit}",
            "icon": "mdi:currency-eur",
            "unit_of_measurement": "€"
        }
...
class Inventory:
    ...
    def add_item(self, name, value=0):
        name = _norm(name)
        if name not in self.db:
            self.db[name] = {"value": 0,
